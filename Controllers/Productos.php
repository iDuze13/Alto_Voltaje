...
class Productos extends Controllers {
    ...
    public function Productos() {
        ...
        // Verificar permiso de eliminación para pasar al JavaScript
        $tienePermisoEliminar = false;
        if (!empty($_SESSION['admin'])) {
            $tienePermisoEliminar = true;
        } elseif (isset($_SESSION['permisos_modulos'][4]) && $_SESSION['permisos_modulos'][4]['d'] == 1) {
            $tienePermisoEliminar = true;
        }
    ...
    public function setProducto() {
        ...
        try {
            if ($_POST) {
                ...
                if ($idProducto == 0) {
                    // Crear - verificar permiso de escritura (w)
                    if (empty($_SESSION['admin']) && !(isset($_SESSION['permisos_modulos'][4]) && $_SESSION['permisos_modulos'][4]['w'] == 1)) {
                        $arrResponse = array('status' => false, 'msg' => 'No tiene permisos para crear productos.');
                        ob_clean();
                        echo json_encode($arrResponse, JSON_UNESCAPED_UNICODE);
                        die();
                    }
                } else {
                    // Actualizar - verificar permiso de actualización (u)
                    if (empty($_SESSION['admin']) && !(isset($_SESSION['permisos_modulos'][4]) && $_SESSION['permisos_modulos'][4]['u'] == 1)) {
                        $arrResponse = array('status' => false, 'msg' => 'No tiene permisos para actualizar productos.');
                        ob_clean();
                        echo json_encode($arrResponse, JSON_UNESCAPED_UNICODE);
                        die();
                    }
    ...
    public function delProducto() {
        ...
        // Verificar permisos de eliminación (módulo 4 = Productos, permiso d = delete)
        $tienePermisoEliminar = false;
        if (!empty($_SESSION['admin'])) {
            $tienePermisoEliminar = true; // Admin tiene todos los permisos
        } elseif (isset($_SESSION['permisos_modulos'][4]) && $_SESSION['permisos_modulos'][4]['d'] == 1) {
            $tienePermisoEliminar = true; // Verificar permiso de eliminación
        }
        
        if (!$tienePermisoEliminar) {
            $arrResponse = array('status' => false, 'msg' => 'No tiene permisos para eliminar productos.');
            header('Content-Type: application/json');
            echo json_encode($arrResponse, JSON_UNESCAPED_UNICODE);
            die();
        }
