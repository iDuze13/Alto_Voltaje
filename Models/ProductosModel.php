...
class ProductosModel extends Msql
{
    ...
    public function obtenerProductosActivos(): array
    {
        ...
        if ($existingTables['subcategoria'] && $existingTables['categoria'] && $existingTables['proveedor']) {
            // Consulta completa con todos los JOINs
            $sql = "SELECT p.*, 
                           COALESCE(c.nombre, 'Sin Categoría') as Nombre_Categoria, 
                           COALESCE(sc.Nombre_SubCategoria, 'Sin Subcategoría') as Nombre_SubCategoria, 
                           COALESCE(pr.Nombre_Proveedor, 'Sin Proveedor') as Nombre_Proveedor
                    FROM producto p
                    LEFT JOIN subcategoria sc ON p.SubCategoria_idSubCategoria = sc.idSubCategoria
                    LEFT JOIN categoria c ON sc.categoria_idcategoria = c.idcategoria
                    LEFT JOIN proveedor pr ON p.Proveedor_id_Proveedor = pr.id_Proveedor
        ...
        } elseif ($existingTables['proveedor']) {
            // Solo con proveedores
            $sql = "SELECT p.*, 
            ...
            // Consulta básica solo productos
            $sql = "SELECT p.*, 
                           'Sin Categoría' as Nombre_Categoria, 
                           'Sin Subcategoría' as Nombre_SubCategoria, 
                           'Sin Proveedor' as Nombre_Proveedor
                    FROM producto p
                    WHERE p.Estado_Producto = 'Activo' AND p.Stock_Actual > 0
                    ORDER BY p.Es_Destacado DESC, p.idProducto DESC";
        }
     ...
     * @return array Array con el estado de existencia de cada tabla
     */
    private function checkExistingTables(): array
    {
        $tables = ['subcategoria', 'categoria', 'proveedor'];
        $existing = [];
        
        foreach ($tables as $table) {
            try {
                $result = $this->select_all("SHOW TABLES LIKE '$table'");
                $existing[$table] = !empty($result);
            } catch (Exception $e) {
    ...
    /**
     * Obtener productos activos - versión simplificada de emergencia
     * @return array Lista de productos activos
     */
    public function obtenerProductosActivosSimple(): array
    {
        // Primero verificar qué columnas existen en la tabla producto
        $columns = $this->getProductoColumns();
        
        // Construir la consulta SELECT basada en las columnas existentes
        $selectFields = [];
        $requiredFields = [
        ...
        foreach ($requiredFields as $field => $alias) {
        ...
        foreach ($optionalFields as $field => $alias) {
            if (in_array($field, $columns)) {
                $selectFields[] = $field;
            } else {
                $selectFields[] = "NULL as $alias";
        ...
        $sql = "SELECT " . implode(', ', $selectFields) . "
                FROM producto 
                WHERE " . (in_array('Estado_Producto', $columns) ? "Estado_Producto = 'Activo'" : "1=1") . "
                  AND " . (in_array('Stock_Actual', $columns) ? "Stock_Actual > 0" : "1=1") . "
                ORDER BY " . (in_array('Es_Destacado', $columns) ? "Es_Destacado DESC, " : "") . "idProducto DESC";
        
        $productos = $this->select_all($sql) ?: [];
        
        // Agregar campos faltantes
        foreach ($productos as &$producto) {
            $producto['Nombre_Categoria'] = 'General';
            $producto['Nombre_SubCategoria'] = 'Productos';  
            $producto['Nombre_Proveedor'] = 'Alto Voltaje';
            
            // Asegurar que los campos opcionales tengan valores por defecto
            if (!isset($producto['imagen'])) $producto['imagen'] = null;
            if (!isset($producto['ruta'])) $producto['ruta'] = null;
            if (!isset($producto['Precio_Oferta'])) $producto['Precio_Oferta'] = null;
            if (!isset($producto['En_Oferta'])) $producto['En_Oferta'] = 0;
            if (!isset($producto['Es_Destacado'])) $producto['Es_Destacado'] = 0;
        }
    ...
    {
        try {
            // Consulta mínima que debería funcionar siempre
            $sql = "SELECT * FROM producto LIMIT 10";
            $productos = $this->select_all($sql) ?: [];
            
            // Agregar campos faltantes que espera la vista
            foreach ($productos as &$producto) {
                if (!isset($producto['Nombre_Categoria'])) $producto['Nombre_Categoria'] = 'General';
                if (!isset($producto['Nombre_SubCategoria'])) $producto['Nombre_SubCategoria'] = 'Productos';
                if (!isset($producto['Nombre_Proveedor'])) $producto['Nombre_Proveedor'] = 'Alto Voltaje';
                if (!isset($producto['imagen'])) $producto['imagen'] = null;
                if (!isset($producto['ruta'])) $producto['ruta'] = null;
                if (!isset($producto['Precio_Oferta'])) $producto['Precio_Oferta'] = null;
                if (!isset($producto['En_Oferta'])) $producto['En_Oferta'] = 0;
    ...
    /**
     * Obtener las columnas existentes en la tabla producto
     * @return array Lista de nombres de columnas
     */
    private function getProductoColumns(): array
    {
        try {
            $result = $this->select_all("SHOW COLUMNS FROM producto");
            $columns = [];
            ...
            // Si falla, devolver las columnas básicas esperadas
            return ['idProducto', 'Nombre_Producto', 'Descripcion_Producto', 'Marca', 'Precio_Venta', 'Stock_Actual'];
        }
    }
    ...
    public function obtenerMarcasUnicas(): array
    {
        $sql = "SELECT DISTINCT Marca 
                FROM producto 
                WHERE Estado_Producto = 'Activo' AND Marca IS NOT NULL AND Marca != ''
                ORDER BY Marca ASC";
        return $this->select_all($sql) ?: [];
    }
}
