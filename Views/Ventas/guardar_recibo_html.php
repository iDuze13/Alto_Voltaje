...
function guardarReciboHTML($datosVenta) {
    // Ruta absoluta a la carpeta recibos (un nivel arriba de views)
    $directorioRecibos = dirname(__DIR__) . '/recibos';
    ...
    function numeroALetras($numero) {
        ...
        function convertirGrupo($num) {
            global $unidades, $decenas, $especiales, $centenas;
    ...
    foreach ($datosVenta['productos'] as $item) {
        $nombreProducto = htmlspecialchars($item['producto']['Nombre_Producto']);
        $cantidad = $item['cantidad'];
        $precioUnit = number_format($item['precio_unitario'], 2, ',', '.');
...
<body>
    <div class="recibo">
        ...
        <div class="seccion">
            <p style="margin: 0; font-size: 12px;"><strong>Emitido por:</strong> ' . htmlspecialchars($datosVenta['empleado_nombre']) . ' (ID: ' . $datosVenta['empleado_id'] . ')</p>
        </div>
    ...
    // Log para debug
    error_log("Intentando guardar recibo en: " . $rutaCompleta);
    
    $resultado = file_put_contents($rutaCompleta, $html);
    
    if ($resultado !== false) {
        error_log("Recibo guardado exitosamente: " . $rutaCompleta);
        return [
            'success' => true,
            'ruta' => $rutaCompleta,
            'nombre_archivo' => $nombreArchivo,
            'url_descarga' => '../recibos/' . $anio . '/' . $mes . '/' . $nombreArchivo
        ];
    } else {
        error_log("Error al guardar recibo en: " . $rutaCompleta);
        return [
            'success' => false,
            'error' => 'No se pudo guardar el archivo en: ' . $rutaCompleta
        ];
    }
}
?>