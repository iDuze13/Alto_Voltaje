					...
					<div class="right-top-bar flex-w h-full">
						...
						<?php if (!empty($_SESSION['admin']) || !empty($_SESSION['empleado']) || !empty($_SESSION['usuario'])): ?>
							<span class="flex-c-m trans-04 p-lr-25">
								Hola, <?= htmlspecialchars($_SESSION['admin']['nombre'] ?? $_SESSION['empleado']['nombre'] ?? ($_SESSION['usuario']['nombre'] ?? 'Usuario')) ?>
							</span>
							<a href="<?= BASE_URL ?>/auth/logout" class="flex-c-m trans-04 p-lr-25">
								Cerrar Sesión
							</a>
						<?php else: ?>
							<a href="<?= BASE_URL ?>/auth/login" class="flex-c-m trans-04 p-lr-25">
								Iniciar sesión
							</a>
						<?php endif; ?>
					</div>
			...
			<ul class="topbar-mobile">
				...
				<li>
					<div class="right-top-bar flex-w h-full">
						...
						<?php if (!empty($_SESSION['admin']) || !empty($_SESSION['empleado']) || !empty($_SESSION['usuario'])): ?>
							<span class="flex-c-m p-lr-10 trans-04">
								Hola, <?= htmlspecialchars($_SESSION['admin']['nombre'] ?? $_SESSION['empleado']['nombre'] ?? ($_SESSION['usuario']['nombre'] ?? 'Usuario')) ?>
							</span>
							<a href="<?= BASE_URL ?>/auth/logout" class="flex-c-m p-lr-10 trans-04">Cerrar Sesión</a>
						<?php else: ?>
							<a href="<?= BASE_URL ?>/auth/login" class="flex-c-m p-lr-10 trans-04">Iniciar sesión</a>
						<?php endif; ?>
					</div>
				</li>
			</ul>
